...
public class BookingServiceImpl implements IBookingService {
	...
	@Override
	public double calculateTotalCost(int bookingid,int customerId) {
		Customer custo = custoRepository.getOne(11);
		...
		booking.getCustomer().getWallet().setBalance(booking.getCustomer().getWallet().getBalance() - amount);
		custo.getWallet().setBalance(custo.getWallet().getBalance() + amount);
		bookingRepository.saveAndFlush(booking);
		custoRepository.saveAndFlush(custo);
		custoRepository.saveAndFlush(booking.getCustomer());
		return amount;
	}
